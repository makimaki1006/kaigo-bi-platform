# -*- coding: utf-8 -*-
"""
きらケア・看護のお仕事 フルインポート処理
- 全カラムマッピング
- 人口データ付与
"""
import pandas as pd
import re
import sys
import io
from pathlib import Path
from datetime import datetime

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
sys.path.insert(0, str(Path(__file__).parent.parent))

from src.api.salesforce_client import SalesforceClient
import requests

BASE_DIR = Path(__file__).parent.parent
OUTPUT_DIR = BASE_DIR / 'data' / 'output' / 'media_matching'
TODAY = datetime.now().strftime('%Y%m%d')
TODAY_DATE = datetime.now().strftime('%Y-%m-%d')
BATCH_ID = f'BATCH_{TODAY}_KIRACARE_KANGOOSHIGOTO'

def normalize_phone(phone):
    """電話番号を正規化"""
    if pd.isna(phone) or phone == '' or str(phone) == 'nan':
        return None
    phone_str = str(phone).strip()
    # "電話番号： xxx" 形式から抽出
    phone_str = re.sub(r'^電話番号[：:]\s*', '', phone_str)
    if phone_str.startswith('所在地'):
        return None
    # 数字のみ抽出
    digits = re.sub(r'\D', '', phone_str)
    # 先頭0が欠けている場合補完
    if len(digits) == 10 and not digits.startswith('0'):
        digits = '0' + digits
    if 10 <= len(digits) <= 11:
        return digits
    return None

def is_mobile_phone(phone):
    """携帯電話番号かどうか判定"""
    if not phone:
        return False
    return phone.startswith(('090', '080', '070'))

def extract_prefecture(address):
    """住所から都道府県を抽出"""
    if pd.isna(address) or not address:
        return None
    prefectures = [
        '北海道', '青森県', '岩手県', '宮城県', '秋田県', '山形県', '福島県',
        '茨城県', '栃木県', '群馬県', '埼玉県', '千葉県', '東京都', '神奈川県',
        '新潟県', '富山県', '石川県', '福井県', '山梨県', '長野県', '岐阜県',
        '静岡県', '愛知県', '三重県', '滋賀県', '京都府', '大阪府', '兵庫県',
        '奈良県', '和歌山県', '鳥取県', '島根県', '岡山県', '広島県', '山口県',
        '徳島県', '香川県', '愛媛県', '高知県', '福岡県', '佐賀県', '長崎県',
        '熊本県', '大分県', '宮崎県', '鹿児島県', '沖縄県'
    ]
    for pref in prefectures:
        if address.startswith(pref):
            return pref
    return None

def extract_city(address, prefecture):
    """住所から市区町村を抽出"""
    if not address or not prefecture:
        return None
    # 都道府県を除去
    remaining = address[len(prefecture):]
    # 市区町村パターン
    match = re.match(r'^(.+?[市区町村])', remaining)
    if match:
        return match.group(1)
    return None

def load_population_data():
    """人口データを読み込み"""
    pop_file = BASE_DIR / 'data' / 'output' / 'population' / 'municipality_population_density.csv'
    df_pop = pd.read_csv(pop_file, dtype=str, encoding='utf-8-sig')
    # 数値変換
    df_pop['population'] = pd.to_numeric(df_pop['population'], errors='coerce')
    df_pop['population_density_m2'] = pd.to_numeric(df_pop['population_density_m2'], errors='coerce')
    return df_pop

def get_population_for_address(address, df_pop):
    """住所から人口と人口密度を取得"""
    prefecture = extract_prefecture(address)
    if not prefecture:
        return None, None

    city = extract_city(address, prefecture)

    # キー作成（都道府県+市区町村）
    if city:
        key = prefecture + city
        match = df_pop[df_pop['key'] == key]
        if not match.empty:
            return match.iloc[0]['population'], match.iloc[0]['population_density_m2']

    # 市区町村マッチしない場合は都道府県のみで検索
    key_pref = prefecture
    match = df_pop[df_pop['key'] == key_pref]
    if not match.empty:
        return match.iloc[0]['population'], match.iloc[0]['population_density_m2']

    return None, None

def process_kiracare(df, df_pop):
    """きらケアデータを処理"""
    records = []
    for _, row in df.iterrows():
        phone_raw = row.get('Google電話番号', '')
        phone = normalize_phone(phone_raw)
        if not phone:
            continue

        address = row.get('p-jobCard__address', '')
        prefecture = extract_prefecture(address)
        population, pop_density = get_population_for_address(address, df_pop)

        # 更新日から日付抽出
        update_text = row.get('p-jobCard__updateDate', '')
        update_date = None
        date_match = re.search(r'(\d{4}/\d{2}/\d{2})', str(update_text))
        if date_match:
            update_date = date_match.group(1).replace('/', '-')

        record = {
            'phone_normalized': phone,
            'Company': row.get('p-jobCard__officeNameHead', ''),
            'CompanyName__c': row.get('p-jobCard__ownerNameHead', ''),
            'Street': address,
            'Prefecture__c': prefecture,
            'Paid_URL__c': row.get('p-jobCard__link href', ''),
            'Paid_RecruitmentType__c': row.get('p-jobCard__recruitTtl', ''),
            'Paid_EmploymentType__c': row.get('c-tag__status', ''),
            'LeadSource': 'きらケア',
            'Paid_Media__c': 'きらケア',
            'Paid_DataSource__c': 'きらケア',
            'Paid_DataExportDate__c': TODAY_DATE,
            'Population__c': population,
            'PopulationDensity__c': pop_density,
            'source': 'kiracare'
        }
        records.append(record)

    return pd.DataFrame(records)

def process_kango(df, df_pop):
    """看護のお仕事データを処理"""
    records = []
    for _, row in df.iterrows():
        phone_raw = row.get('Google電話番号', '')
        phone = normalize_phone(phone_raw)
        if not phone:
            continue

        # 住所は市区町村レベルのみ
        address_short = row.get('p-jobCard__infoAddress', '')
        # 法人名 + 事業所名から住所を推測
        combined = row.get('合わせ', '')
        # 「法人名　事業所名　住所」形式
        parts = str(combined).split('　') if combined else []
        full_address = parts[-1] if len(parts) >= 3 else address_short

        prefecture = extract_prefecture(full_address) or extract_prefecture(address_short)
        population, pop_density = get_population_for_address(full_address, df_pop)

        # 施設形態
        facility_type = row.get('p-jobCard__headerOfficeInfoItem (2)', '')
        # 職種
        job_type = row.get('p-jobCard__headerTag (2)', '')
        # 雇用形態
        employment_type = row.get('p-jobCard__headerTag (3)', '')

        record = {
            'phone_normalized': phone,
            'Company': row.get('p-jobCard__headerOfficeName', ''),
            'CompanyName__c': row.get('p-jobCard__headerOfficeInfoItem', ''),
            'Street': full_address,
            'Prefecture__c': prefecture,
            'Paid_URL__c': row.get('p-jobCard__linkWrap href', ''),
            'Paid_RecruitmentType__c': job_type,
            'Paid_EmploymentType__c': employment_type,
            'Paid_Industry__c': facility_type,
            'LeadSource': '看護のお仕事',
            'Paid_Media__c': '看護のお仕事',
            'Paid_DataSource__c': '看護のお仕事',
            'Paid_DataExportDate__c': TODAY_DATE,
            'Population__c': population,
            'PopulationDensity__c': pop_density,
            'source': 'kango'
        }
        records.append(record)

    return pd.DataFrame(records)

def create_memo(row, is_new=True):
    """メモ欄作成"""
    source = row.get('Paid_Media__c', '')
    address = row.get('Street', '')
    job_type = row.get('Paid_RecruitmentType__c', '')
    emp_type = row.get('Paid_EmploymentType__c', '')

    prefix = '【新規作成】' if is_new else '【既存更新】'

    memo = f"""{prefix}
【{BATCH_ID}】
【{source}】
住所: {address}
職種: {job_type}
雇用形態: {emp_type}
取得日: {TODAY_DATE}"""
    return memo

def main():
    print('=' * 60)
    print('きらケア・看護のお仕事 フルインポート処理')
    print('=' * 60)
    print()

    # Salesforce認証
    client = SalesforceClient()
    client.authenticate()
    print()

    # 人口データ読み込み
    print('1. 人口データ読み込み')
    df_pop = load_population_data()
    print(f'   市区町村: {len(df_pop)}件')
    print()

    # 元データ読み込み
    print('2. 元データ読み込み')
    df_kira = pd.read_csv(r'C:\Users\fuji1\Downloads\kiracare-2026-01-14-with-phone.csv', dtype=str)
    df_kango = pd.read_csv(r'C:\Users\fuji1\Downloads\kango-oshigoto-2026-01-14-with-phone.csv', dtype=str)
    print(f'   きらケア: {len(df_kira)}件')
    print(f'   看護のお仕事: {len(df_kango)}件')
    print()

    # データ処理
    print('3. データ処理・人口付与')
    df_kira_processed = process_kiracare(df_kira, df_pop)
    df_kango_processed = process_kango(df_kango, df_pop)
    print(f'   きらケア処理後: {len(df_kira_processed)}件')
    print(f'   看護処理後: {len(df_kango_processed)}件')

    # 統合・重複除去
    df_combined = pd.concat([df_kira_processed, df_kango_processed], ignore_index=True)
    df_combined = df_combined.drop_duplicates(subset=['phone_normalized'], keep='first')
    print(f'   統合・重複除去後: {len(df_combined)}件')

    # 人口付与率
    pop_filled = df_combined['Population__c'].notna().sum()
    print(f'   人口付与率: {pop_filled}/{len(df_combined)} ({pop_filled/len(df_combined)*100:.1f}%)')
    print()

    # 最新Salesforceデータ読み込み
    print('4. 最新Salesforceデータ読み込み')
    df_acc = pd.read_csv(BASE_DIR / 'data' / 'output' / 'Account_phones_20260116_full.csv', dtype=str)
    df_lead_all = pd.read_csv(BASE_DIR / 'data' / 'output' / 'Lead_phones_20260116.csv', dtype=str)

    # 今日作成したリードを除外（重複削除後の残り614件）
    df_created = pd.read_csv(OUTPUT_DIR / 'created_lead_ids_20260116_kiracare_kango.csv', dtype=str)
    created_ids = set(df_created['sf__Id'].tolist())
    # 削除した7件も除外対象に
    df_deleted = pd.read_csv(OUTPUT_DIR / 'duplicate_leads_to_delete_20260116.csv', dtype=str)
    deleted_ids = set(df_deleted['Id'].tolist())
    exclude_ids = created_ids | deleted_ids

    df_lead = df_lead_all[~df_lead_all['Id'].isin(exclude_ids)]
    print(f'   Account: {len(df_acc):,}件')
    print(f'   Lead（除外後）: {len(df_lead):,}件')
    print()

    # 電話番号セット作成
    acc_phones = {}
    for _, row in df_acc.iterrows():
        for col in ['Phone', 'Phone2__c']:
            p = normalize_phone(row.get(col))
            if p:
                acc_phones[p] = row['Id']

    lead_phones = {}
    for _, row in df_lead.iterrows():
        for col in ['Phone', 'MobilePhone']:
            p = normalize_phone(row.get(col))
            if p:
                lead_phones[p] = row['Id']

    print(f'   Account電話番号: {len(acc_phones):,}件')
    print(f'   Lead電話番号: {len(lead_phones):,}件')
    print()

    # 成約先除外
    print('5. 成約先除外')
    contract_file = BASE_DIR / 'data' / 'output' / 'hellowork' / 'contract_accounts_with_corp.csv'
    df_contract = pd.read_csv(contract_file, dtype=str)
    contract_phones = set()
    for col in ['Phone', 'Phone2__c']:
        if col in df_contract.columns:
            phones = df_contract[col].apply(normalize_phone)
            contract_phones.update(phones[phones.notna()].tolist())
    print(f'   成約先電話番号: {len(contract_phones):,}件')

    # 成約先除外
    df_combined['is_contract'] = df_combined['phone_normalized'].isin(contract_phones)
    excluded = df_combined[df_combined['is_contract']]
    df_combined = df_combined[~df_combined['is_contract']]
    print(f'   成約先除外: {len(excluded)}件')
    print(f'   除外後: {len(df_combined)}件')
    print()

    # 突合
    print('6. 突合')
    account_matches = []
    lead_matches = []
    new_leads = []

    for _, row in df_combined.iterrows():
        phone = row['phone_normalized']

        if phone in acc_phones:
            # Account更新
            row_dict = row.to_dict()
            row_dict['Id'] = acc_phones[phone]
            account_matches.append(row_dict)
        elif phone in lead_phones:
            # Lead更新
            row_dict = row.to_dict()
            row_dict['Id'] = lead_phones[phone]
            lead_matches.append(row_dict)
        else:
            # 新規リード
            new_leads.append(row.to_dict())

    print(f'   Account一致: {len(account_matches)}件')
    print(f'   Lead一致: {len(lead_matches)}件')
    print(f'   新規リード: {len(new_leads)}件')
    print()

    # CSV生成
    print('7. CSV生成')
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # Account更新CSV
    if account_matches:
        df_acc_update = pd.DataFrame(account_matches)
        df_acc_update['Paid_Memo__c'] = df_acc_update.apply(lambda r: create_memo(r, is_new=False), axis=1)

        acc_columns = ['Id', 'Paid_Media__c', 'Paid_Memo__c', 'Paid_DataExportDate__c',
                       'Paid_DataSource__c', 'Paid_URL__c', 'Paid_RecruitmentType__c',
                       'Paid_EmploymentType__c', 'Population__c', 'PopulationDensity__c']
        df_acc_out = df_acc_update[[c for c in acc_columns if c in df_acc_update.columns]]
        df_acc_out.to_csv(OUTPUT_DIR / f'kiracare_kango_account_updates_{TODAY}_v2.csv',
                          index=False, encoding='utf-8-sig')
        print(f'   Account更新: {len(df_acc_out)}件')

    # Lead更新CSV
    if lead_matches:
        df_lead_update = pd.DataFrame(lead_matches)
        df_lead_update['Paid_Memo__c'] = df_lead_update.apply(lambda r: create_memo(r, is_new=False), axis=1)

        lead_update_columns = ['Id', 'Paid_Media__c', 'Paid_Memo__c', 'Paid_DataExportDate__c',
                               'Paid_DataSource__c', 'Paid_URL__c', 'Paid_RecruitmentType__c',
                               'Paid_EmploymentType__c', 'Population__c', 'PopulationDensity__c']
        df_lead_out = df_lead_update[[c for c in lead_update_columns if c in df_lead_update.columns]]
        df_lead_out.to_csv(OUTPUT_DIR / f'kiracare_kango_lead_updates_{TODAY}_v2.csv',
                           index=False, encoding='utf-8-sig')
        print(f'   Lead更新: {len(df_lead_out)}件')

    # 新規リードCSV
    if new_leads:
        df_new = pd.DataFrame(new_leads)
        df_new['Paid_Memo__c'] = df_new.apply(lambda r: create_memo(r, is_new=True), axis=1)
        df_new['LastName'] = '担当者'

        # Phone設定（携帯でもPhoneに入れる）
        df_new['Phone'] = df_new['phone_normalized']
        df_new['MobilePhone'] = df_new.apply(
            lambda r: r['phone_normalized'] if is_mobile_phone(r['phone_normalized']) else '', axis=1)

        new_lead_columns = ['Company', 'LastName', 'Phone', 'MobilePhone', 'Street', 'Prefecture__c',
                            'CompanyName__c', 'LeadSource', 'Paid_Media__c', 'Paid_Memo__c',
                            'Paid_DataExportDate__c', 'Paid_DataSource__c', 'Paid_URL__c',
                            'Paid_RecruitmentType__c', 'Paid_EmploymentType__c', 'Paid_Industry__c',
                            'Population__c', 'PopulationDensity__c']
        df_new_out = df_new[[c for c in new_lead_columns if c in df_new.columns]]

        # 必須チェック
        df_new_out = df_new_out[df_new_out['Company'].notna() & (df_new_out['Company'] != '')]
        df_new_out = df_new_out[df_new_out['Phone'].notna() & (df_new_out['Phone'] != '')]

        df_new_out.to_csv(OUTPUT_DIR / f'kiracare_kango_new_leads_{TODAY}_v2.csv',
                          index=False, encoding='utf-8-sig')
        print(f'   新規リード: {len(df_new_out)}件')

    print()
    print('=' * 60)
    print('処理完了')
    print('=' * 60)
    print()
    print('出力ファイル:')
    print(f'  - kiracare_kango_account_updates_{TODAY}_v2.csv')
    print(f'  - kiracare_kango_lead_updates_{TODAY}_v2.csv')
    print(f'  - kiracare_kango_new_leads_{TODAY}_v2.csv')

if __name__ == '__main__':
    main()
